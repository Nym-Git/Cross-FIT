    ...
    try:
        ...
        if user.check_password(password):
            ...
            if user.session_token != "0":
                ...
                user.save()
                return JsonResponse({'error': "Previous session exists! try again please..."})
...
'''@csrf_exempt
def Auth_Update(request, id, token):
    if not request.method == 'PUT':
        return JsonResponse({'error': 'please launch valid request'})
        ...
        if UserModel.session_token == token:
